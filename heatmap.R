...
hm_anno_df <- hm_df %>% 
  ...
  separate(rowname, into = c("Realm", "Kingdom", "Phylum", "Class", "Order", "Family", "n"), 
...
type_col <- c("Eukaryotic virus"="black", 
...
hm_anno_df <- hm_anno_df %>% 
  mutate(type=case_when(Family_all %in% phages ~ "Bacteriophage", 
...
right_ra <- rowAnnotation(
  'Virus type'=anno_simple(hm_anno_df$type, col = type_col,
                           width = unit(1, "mm")),
...
lgd_type <- Legend(labels = gt_render(names(type_col)), 
