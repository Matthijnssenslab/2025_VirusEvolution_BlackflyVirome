...
variance <- summary(rda_pca)$importance[2, ]
pc1 <- variance[1]
pc2 <- variance[2]
...
joined_rda_df <- as.data.frame(rda_pca$rotation) %>%
  ...
  mutate(
    Host.x = coalesce(Host.x, Host.y),
    Host.x = if_else(Host.x == "vertebrates", "vertebrate", Host.x),
    `Virus name abbreviation(s)` = case_when(
      startsWith(acc, "Blackfly") == T ~ acc,
...
rhabdo_pca_plot <- joined_rda_df %>%
  ...
  stat_density_2d(
    data = . %>% filter(!is.na(Host.x)), aes(fill = Host.x),
  ...
  labs(
    x = glue::glue("PC1 ({round(pc1 * 100, 2)}%)"),
    y = glue::glue("PC2 ({round(pc2 * 100, 2)}%)")
  ) +
  theme_bw()
rhabdo_pca_plot
...
rhabdo_pca_plot2 <- rhabdo_pca_plot +
  ...
  ggrepel::geom_label_repel(
    data = . %>% filter(str_starts(acc, "OVRV1|Blackfly|NC_077192") | str_starts(`Virus name abbreviation(s)`, "RABV|CHPV|LBV")), aes(label = gsub("_", " ", `Virus name abbreviation(s)`)),
...
rhabdo_sdu_pca_plot2 <- rhabdo_sdu_pca_plot +
  ...
  ggrepel::geom_label_repel(
    data = . %>% filter(str_starts(acc, "OVRV1|blackfly|NC_077192")), aes(label = acc),
...
variance <- summary(rda_pca_flavi)$importance[2, ]
pc1 <- variance[1]
pc2 <- variance[2]
...
flavi_rda_pca_plot <- joined_rda_df_flavi %>%
  ...
  ggrepel::geom_label_repel(
    data = . %>% filter(str_starts(`Virus name abbreviation(s)`, "WNV|ZIKV|JEV|TBEV")), aes(label = `Virus name abbreviation(s)`),
...
# ggsave("figures/tiff/figure4.pdf", dpi = 300, width = 7, height = 4.5)
...
leg <- rbind(
  joined_rda_df_flavi %>% rename("Virus accession" = `Virus REFSEQ accession`),
...
leg2 <- ggpubr::get_legend(addSmallLegend(leg, pointSize = 2, textSize = 8))
p2 <- p +
  plot_layout(axis_titles = "collect", guides = "collect") +
  plot_annotation(tag_levels = "A") &
  theme(
...
p3 <- ggpubr::as_ggplot(leg2)

# Plot combined + custom legend
p2 / p3 + plot_layout(heights = c(1, .1)) + plot_annotation(tag_levels = list(c("A", "B", "")))
ggsave("figures/pdf/figure4.pdf", dpi = 300, width = 7, height = 4.5)
ggsave("figures/tiff/figure4.tiff", dpi = 300, width = 7, height = 4.5)
